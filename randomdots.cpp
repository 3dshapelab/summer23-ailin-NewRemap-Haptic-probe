...
void generateRandomDots(double shapeDepth, int dotNumPerRow, int dotNumPerCol, double dotJitterMax_Scale, vector<Vector3d>&dotContainer);
void buildContour_wavy(double shapeDepth, ContourData& new_contours_vert);
void buildRandomDotSurface(double shapeDepth, VerticesData& vertices_data, vector<Vector3d>& dotContainer, ContourData& new_contours_vert);
...
void initStimulus() {
	...
	buildRandomDotSurface(depth_disp, my_verts, dot_container,my_contour_data);
	stimulus_built = true;
}
	...
	for (int i_wave = 0; i_wave < nr_wave_perSeg; i_wave++) {
		...
		if (i_wave % 2 == 0) {
			...
			for (int i_step = 0; i_step < nr_unit_current * nr_step_unit; i_step++) {
				...
				z = getZ(stimulus_height, shapeDepth, y);
...
void buildRandomDotSurface(double shapeDepth, VerticesData& vertices_data, vector<Vector3d>& dotContainer, ContourData& contours_vert ) {
	...
	for (int jj = 0; jj < nr_pts_height; jj++) {
		...
		y = -stimulus_height / 2 + jj * step_size_height;
		z = getZ(stimulus_height, shapeDepth, y);


		for (int ii = 0; ii < nr_pts_width; ii++) {
			x = -long_width / 2 + ii * step_size_width;
	...
	buildContour_wavy(shapeDepth, contours_vert);

	generateRandomDots(shapeDepth, dot_num_per_col, dot_num_per_col, dot_jitter_max_scale, dotContainer);	
...
{
	if (stimulus_built) {
		drawRandomDotSurface(-(display_distance - depth_disp), my_verts, dot_container, my_contour_data);
	}
}
